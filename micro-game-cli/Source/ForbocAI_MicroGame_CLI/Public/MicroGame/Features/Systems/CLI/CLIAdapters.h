...
#include "MicroGame/Features/Systems/CLI/CLITypes.h"
#include "MicroGame/Features/Systems/Harness/Verification/VerificationAdapters.h"

namespace MicroGame::CLI {

/** User Story: As a micro-game CLI maintainer, I need one immutable authored CLI contract so Windows, macOS, and Linux hosts parse the same commands and exit codes. @fn inline const FCliRuntimeData &cliRuntimeData() */
...
/** User Story: As a micro-game CLI operator, I need commandlet argument slots decoded recursively so wrapper scripts can pass an arbitrary command without platform-specific limits. @fn inline TArray<FString> buildCliArgTokensRecursive( const FString &Params, int32 Index, TArray<FString> Tokens) */
...
/** User Story: As a micro-game CLI operator, I need token membership checked recursively so aliases stay authored and parsing remains deterministic. @fn inline bool hasToken(const TArray<FString> &Tokens, const FString &Needle, int32 Index) */
...
/** User Story: As a micro-game CLI operator, I need authored aliases matched recursively so all supported flag spellings share one parser path. @fn inline bool hasAnyToken(const TArray<FString> &Tokens, const TArray<FString> &Needles, int32 Index) */
...
/** User Story: As a micro-game CLI operator, I need assignment-form aliases recognized recursively so transport options are removed without imperative loops. @fn inline bool startsWithAny(const FString &Token, const TArray<FString> &Aliases, int32 Index) */
...
/** User Story: As a micro-game CLI operator, I need both split and assignment flag forms decoded through one adapter so shell choice never changes API configuration. @fn inline FString readFlagValue(const TArray<FString> &Tokens, const TArray<FString> &Aliases, int32 TokenIndex, int32 AliasIndex) */
...
/** User Story: As a micro-game CLI operator, I need transport flags removed recursively so command and mode resolution operate only on semantic CLI tokens. @fn inline TArray<FString> removeFlagsRecursive( const TArray<FString> &Tokens, const TArray<FString> &Aliases, int32 Index, TArray<FString> Acc) */
...
/** User Story: As a micro-game CLI host, I need commandlet parameters converted to semantic tokens through one adapter so host wiring contains no parsing policy. @fn inline TArray<FString> buildCliArgTokens(const FString &Params) */
...
/** User Story: As a micro-game CLI host, I need transport configuration separated from semantic arguments so API settings never leak into game-mode dispatch. @fn inline FCliInvocation normalizeInvocation(const TArray<FString> &RawTokens) */
...
/** User Story: As a micro-game CLI host, I need help aliases evaluated from authored data so usage behavior is portable and testable. @fn inline bool requestsHelp(const FCliInvocation &Invocation) */
...
/** User Story: As a micro-game CLI host, I need the authored command position selected safely so empty invocations never rely on sentinel literals. @fn inline FString selectCommand(const FCliInvocation &Invocation) */
...
/** User Story: As a micro-game CLI host, I need mode selection derived from authored flags and runtime domains so invalid values cannot enter the root store. @fn inline FString resolveMode(const FCliInvocation &Invocation) */
inline FString resolveMode(const FCliInvocation &Invocation) {
  ...
  return !ExplicitMode.IsEmpty()
             ? ExplicitMode
             : VerificationVocabularyAdapters::GameRuntimeData().modes.all.Contains(Command)
                   ? Command
                   : VerificationVocabularyAdapters::GameRuntimeData().modes.autoplay;
}
