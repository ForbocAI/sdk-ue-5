...
/** User Story: As a normalized-state verifier, I need NPC, memory, inventory, and Soul cases decoded atomically so tests only exercise store transitions. @fn inline CrossSdkConformance::FStateDomainsVerificationData ReadStateDomains(const TSharedRef<FJsonObject> &Root) */
inline CrossSdkConformance::FStateDomainsVerificationData
ReadStateDomains(const TSharedRef<FJsonObject> &Root) {
  ...
  CrossSdkConformance::FStateDomainsVerificationData Data;
