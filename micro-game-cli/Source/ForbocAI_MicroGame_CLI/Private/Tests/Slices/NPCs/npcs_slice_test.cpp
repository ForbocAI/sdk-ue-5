...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FMicroGameNpcPatchAndSelectorsTest,
    CrossSdkConformanceAdapters::CrossSdkConformanceData()
        .stateDomains.automationNames.npc,
    EAutomationTestFlags_ApplicationContextMask |
        EAutomationTestFlags::EngineFilter)

/** User Story: As a tests slices npcs consumer, I need to invoke run test through a stable signature so the tests slices npcs workflow remains explicit and composable. @fn bool FMicroGameNpcPatchAndSelectorsTest::RunTest(const FString &Parameters) */
bool FMicroGameNpcPatchAndSelectorsTest::RunTest(const FString &Parameters) {
  (void)Parameters;
