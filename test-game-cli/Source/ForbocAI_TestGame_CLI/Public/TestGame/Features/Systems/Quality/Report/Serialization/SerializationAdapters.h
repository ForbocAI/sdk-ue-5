...
/** User Story: As a release operator, I need a report converted into the same stable JSON contract used by the TypeScript test game. @fn inline TSharedRef<FJsonObject> qualityReportObject(const FQualityReport &Report) */
inline TSharedRef<FJsonObject>
