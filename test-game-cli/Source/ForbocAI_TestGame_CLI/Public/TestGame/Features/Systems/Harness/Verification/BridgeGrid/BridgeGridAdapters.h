...
/** User Story: As a bridge/grid verifier, I need its authored case decoded atomically so input, stories, and expected outcomes cannot drift independently. @fn inline Verification::FBridgeGridVerificationData ReadBridgeGrid(const TSharedRef<FJsonObject> &Root) */
inline Verification::FBridgeGridVerificationData
ReadBridgeGrid(const TSharedRef<FJsonObject> &Root) {
  ...
  Verification::FBridgeGridVerificationData Data;
