...
bool FNPCSliceSetInfoTest::RunTest(const FString &Parameters) {
  ...
  State = NpcSlice.Reducer(State, NPCActions::setNPCInfo(Info));

  TestEqual("ActiveNpcId set", State.ActiveNpcId, FString(TEXT("npc_001")));

  func::Maybe<FNPCInternalState> Found = NPCSelectors::selectNPCById(State, TEXT("npc_001"));
  ...
  State = NpcSlice.Reducer(State, NPCActions::setNPCInfo(Info2));

  TestEqual("ActiveNpcId updated to second", State.ActiveNpcId,
            FString(TEXT("npc_002")));

  TArray<FNPCInternalState> AllNpcs = NPCSelectors::selectAllNPCs(State);
...
bool FNPCSliceRemoveTest::RunTest(const FString &Parameters) {
  ...
  TestTrue("NPC exists before removal",
           NPCSelectors::selectNPCById(State, TEXT("npc_rm")).hasValue);
  TestEqual("ActiveNpcId is npc_rm", State.ActiveNpcId,
            FString(TEXT("npc_rm")));

  State = NpcSlice.Reducer(State, NPCActions::removeNPC(TEXT("npc_rm")));
...
/**
 * Test: Selectors — NPCSelectors::selectActiveNPC, NPCSelectors::selectAllNPCs, NPCSelectors::selectNPCById
...
bool FNPCSliceSelectorsTest::RunTest(const FString &Parameters) {
   ...
   * User Story: As a maintainer, I need this section note so related declarations and logic stay easy to locate.
   */
  func::Maybe<FNPCInternalState> EmptyActive = NPCSelectors::selectActiveNPC(State);
  TestFalse("No active NPC on empty state", EmptyActive.hasValue);
  TestEqual("NPCSelectors::selectAllNPCs empty", NPCSelectors::selectAllNPCs(State).Num(), 0);
  TestEqual("NPCSelectors::selectNPCIds empty", NPCSelectors::selectNPCIds(State).Num(), 0);
  TestEqual("NPCSelectors::selectNPCEntities empty", NPCSelectors::selectNPCEntities(State).Num(), 0);
  TestEqual("NPCSelectors::selectTotalNPCs empty", NPCSelectors::selectTotalNPCs(State), 0);
  TestFalse("NPCSelectors::selectNPCById returns nothing on empty",
            NPCSelectors::selectNPCById(State, TEXT("ghost")).hasValue);
  ...
  State = NpcSlice.Reducer(State, NPCActions::setNPCInfo(A));

  FNPCInternalState B;
  B.Id = TEXT("sel_b");
  B.Persona = TEXT("Beta");
  State = NpcSlice.Reducer(State, NPCActions::setNPCInfo(B));

  /**
   * Active should be last set
   * User Story: As a maintainer, I need this note so the surrounding code intent stays clear during maintenance and debugging.
   */
  func::Maybe<FNPCInternalState> Active = NPCSelectors::selectActiveNPC(State);
  ...
  /**
   * NPCSelectors::selectNPCById for first NPC
   * User Story: As a maintainer, I need this note so the surrounding code intent stays clear during maintenance and debugging.
   */
  func::Maybe<FNPCInternalState> FoundA = NPCSelectors::selectNPCById(State, TEXT("sel_a"));
  ...
  /**
   * NPCSelectors::selectAllNPCs
   * User Story: As a maintainer, I need this note so the surrounding code intent stays clear during maintenance and debugging.
   */
  TestEqual("Two NPCs total", NPCSelectors::selectAllNPCs(State).Num(), 2);
  TestEqual("Two NPC ids", NPCSelectors::selectNPCIds(State).Num(), 2);
  TestEqual("Total NPC selector count", NPCSelectors::selectTotalNPCs(State), 2);

  TMap<FString, FNPCInternalState> Entities = NPCSelectors::selectNPCEntities(State);
   ...
   * User Story: As a maintainer, I need this note so the surrounding code intent stays clear during maintenance and debugging.
   */
  State = NpcSlice.Reducer(State, NPCActions::setActiveNPC(TEXT("sel_a")));
  TestEqual("Active switched to sel_a", NPCSelectors::selectActiveNpcId(State),
            FString(TEXT("sel_a")));
