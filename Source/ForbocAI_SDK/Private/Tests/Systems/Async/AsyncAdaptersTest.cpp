...
#include "Systems/Testing/Async/TestingAsyncAdapters.h"
...
bool FAsyncAdaptersWaitForResultTest::RunTest(const FString &Parameters) {
  ...
  const func::Either<FString, int32> Rejected =
      AsyncAdapters::waitForEither(func::createAsyncResult<int32>(
          [&Data](std::function<void(int32)> Resolve,
                  std::function<void(std::string)> Reject) {
            (void)Resolve;
            Reject(TCHAR_TO_UTF8(*Data.Values.Rejection));
          }));
  TestEqual(*Data.Assertions.Rejection, Rejected.left,
            Data.Values.Rejection);
