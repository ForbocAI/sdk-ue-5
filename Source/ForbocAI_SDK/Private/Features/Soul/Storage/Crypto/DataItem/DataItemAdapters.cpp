...
bool digestSoulBytesAdapter(const EVP_MD *Algorithm,
  ...
  const bool bDigested = Context &&
      EVP_DigestInit_ex(Context.get(), Algorithm, nullptr) == 1 &&
      EVP_DigestUpdate(Context.get(), Bytes.GetData(), Bytes.Num()) == 1 &&
      EVP_DigestFinal_ex(Context.get(), Digest.GetData(), &Length) == 1;
  Digest.SetNum(bDigested ? static_cast<int32>(Length) : 0);
  return bDigested;
}
...
TArray<uint8> deepHashSoulBlobAdapter(const TArray<uint8> &Bytes,
  ...
  Error = bDigested
              ? FString()
              : dataItemOpenSslErrorAdapter(TEXT("Soul deep hash failed"));
  return Result;
}
  ...
  return Index >= Values.Num()
             ...
             : [&]() {
                 ...
                 return bNext && deepHashSoulListStepAdapter(
                                     Values, Index + 1, Accumulator, Error);
               }();
}
...
FString soulDataItemIdAdapter(const TArray<uint8> &DataItem,
  ...
  const bool bSized = DataItem.Num() >= SignatureLength + 2;
  Error = bSized ? FString()
                 : Configuration::soulStorageData().Text.InvalidSignatureLength;
  return !bSized
             ? FString()
             : [&]() {
                 const TArray<uint8> Signature(DataItem.GetData() + 2,
                                               SignatureLength);
                 TArray<uint8> Digest;
                 const bool bDigested = digestSoulBytesAdapter(
...
FString soulPayloadDigestAdapter(const TArray<uint8> &Payload,
  ...
  Error = bDigested ? FString()
                    : dataItemOpenSslErrorAdapter(
                          TEXT("Soul payload digest failed"));
