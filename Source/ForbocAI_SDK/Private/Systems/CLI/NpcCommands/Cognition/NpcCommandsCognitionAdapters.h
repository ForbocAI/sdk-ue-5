...
#include "Systems/CLI/Analysis/AnalysisAdapters.h"
...
#include "Systems/CLI/NPC/Decision/DecisionAdapters.h"
...
/**
 * User Story: As NPC cognition presentation, I need one API response rendered consistently without interpreting its analytical fields.
 * @fn inline FString PresentNpcResponse( const FAgentResponse &Response, const ForbocAI::CLI::NPC::FCLINPCState &State, const FString &DialogueFormat, const FString &ActionFormat, bool bTracePrompt)
 */
inline FString PresentNpcResponse(
  ...
  const TArray<FString> Lines =
      ForbocAI::CLI::Analysis::selectAgentResponseLines(
          Response, State, formatCliMessage(DialogueFormat, Dialogue),
          formatCliMessage(ActionFormat, Response.Action.Type), bTracePrompt);
  ForbocAI::CLI::Analysis::logAgentResponseLines(Lines);
  return ForbocAI::CLI::Analysis::serializeAgentResponseLines(Lines, State);
}

/**
 * User Story: As normal NPC processing, I need CLI process arguments projected into one normal SDK process operation.
 * @fn inline NpcResult ProcessNpc( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::NPC::FCLINPCState &State)
 */
...
/**
 * User Story: As NPC persona composition, I need one authored attribute request delegated to the normal SDK generation endpoint.
 * @fn inline NpcResult GenerateNpcAttribute( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::NPC::FCLINPCState &State, int32 First, int32 Second)
 */
...
/**
 * User Story: As normal NPC constrained choice, I need CLI options projected into one normal SDK decision operation.
 * @fn inline NpcResult DecideNpc( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::NPC::FCLINPCState &State)
 */
...
/**
 * User Story: As normal NPC chat, I need one message projected into the same normal SDK process operation used by other NPC commands.
 * @fn inline NpcResult ChatWithNpc( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::NPC::FCLINPCState &State, int32 First, int32 Second)
 */
