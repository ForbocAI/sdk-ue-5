...
/**
 * User Story: As every session-bound Ghost command, I need the required session identifier checked before exactly one Ghost SDK operation is composed.
 * @fn template <typename Operation> inline GhostResult WithRequiredSession( const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First, Operation Run)
 */
