...
/**
 * User Story: As Ghost lifecycle startup, I need a suite and authored duration option delegated to one Ghost SDK run operation.
 * @fn inline GhostResult RunGhost( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First, int32 Next)
 */
inline GhostResult RunGhost(
  ...
  const int32 DurationOptionIndex = Args.IndexOfByKey(State.DurationOption);
  const int32 DurationValueIndex = DurationOptionIndex + Next;
  const int32 Duration =
      DurationOptionIndex != INDEX_NONE && Args.IsValidIndex(DurationValueIndex)
          ? FCString::Atoi(*Args[DurationValueIndex])
...
/**
 * User Story: As Ghost lifecycle inspection, I need one explicit session delegated to the Ghost SDK status operation.
 * @fn inline GhostResult ReadGhostStatus( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First)
 */
...
/**
 * User Story: As Ghost evidence inspection, I need one explicit session delegated to the Ghost SDK results operation.
 * @fn inline GhostResult ReadGhostResults( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First)
 */
...
/**
 * User Story: As Ghost lifecycle control, I need one explicit session delegated to the Ghost SDK stop operation.
 * @fn inline GhostResult StopGhost( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First)
 */
...
/**
 * User Story: As Ghost history inspection, I need one bounded history request delegated to the Ghost SDK.
 * @fn inline GhostResult ReadGhostHistory( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First)
 */
...
/**
 * User Story: As Ghost lifecycle routing, I need session command roles composed as a focused immutable dispatcher.
 * @fn inline func::Dispatcher<FString, GhostResult> CreateGhostLifecycleDispatcher( rtk::EnhancedStore<FRuntimeState> &Store, const ForbocAI::CLI::FCLICommandRoles &Roles, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &State, int32 First, int32 Second, int32 Next)
 */
inline func::Dispatcher<FString, GhostResult> CreateGhostLifecycleDispatcher(
  ...
  return func::createDispatcher<FString, GhostResult>({
      {Roles.GhostRun,
       [&]() { return RunGhost(Store, Args, State, First, Next); }},
      {Roles.GhostStatus,
