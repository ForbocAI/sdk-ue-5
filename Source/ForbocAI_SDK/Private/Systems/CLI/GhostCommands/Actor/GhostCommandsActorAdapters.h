...
#include "Systems/CLI/GhostCommands/Session/SessionAdapters.h"
...
/**
 * User Story: As Ghost persona composition, I need one attribute request delegated only to the Ghost SDK generation operation.
 * @fn inline GhostResult GenerateGhostAttribute( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &GhostState, const ForbocAI::CLI::NPC::FCLINPCState &ActorState, int32 First, int32 Second, int32 Next)
 */
        ...
        return CLIOps::CommandRouting::matchCondition<GhostResult>(
            ...
            [&]() {
              ...
              ForbocAI::CLI::Presentation::logCliMessage(
                  Serialized);
              return CLIOps::CommandResult::Success(Serialized);
            },
            [&]() {
...
/**
 * User Story: As Ghost actor setup, I need one persona delegated only to the Ghost SDK actor creation operation.
 * @fn inline GhostResult CreateGhostActor( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &GhostState, const ForbocAI::CLI::NPC::FCLINPCState &ActorState, int32 First, int32 Second)
 */
...
/**
 * User Story: As Ghost identity parsing, I need Ghost-facing flags translated into the shared actor vocabulary without changing the caller input.
 * @fn inline TArray<FString> TranslateGhostUpdateArguments( const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &GhostState, int32 Second)
 */
...
/**
 * User Story: As Ghost actor update presentation, I need one SDK result projected without reinterpreting its state transition.
 * @fn inline GhostResult PresentGhostActorUpdate( const FActorUpdateResult &Updated, const ForbocAI::CLI::NPC::FCLINPCUpdate &Decoded, const ForbocAI::CLI::NPC::FCLINPCState &ActorState)
 */
...
/**
 * User Story: As Ghost identity setup, I need decoded state delegated to one Ghost SDK actor update operation.
 * @fn inline GhostResult ApplyGhostActorUpdate( rtk::EnhancedStore<FRuntimeState> &Store, const FString &SessionId, const ForbocAI::CLI::NPC::FCLINPCUpdate &Decoded, const ForbocAI::CLI::NPC::FCLINPCState &ActorState)
 */
...
/**
 * User Story: As Ghost identity setup, I need Ghost-facing identity flags translated before one Ghost SDK actor update operation.
 * @fn inline GhostResult UpdateGhostActor( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &GhostState, const ForbocAI::CLI::NPC::FCLINPCState &ActorState, int32 First, int32 Second)
 */
...
/**
 * User Story: As Ghost actor hydration, I need one actor identifier delegated only to the Ghost SDK recall operation.
 * @fn inline GhostResult RecallGhostActor( rtk::EnhancedStore<FRuntimeState> &Store, const TArray<FString> &Args, const ForbocAI::CLI::Ghost::FCLIGhostState &GhostState, const ForbocAI::CLI::NPC::FCLINPCState &ActorState, int32 First, int32 Second)
 */
