...
/** User Story: As a features cli consumer, I need to invoke read command spec through a stable signature so the features cli workflow remains explicit and composable. @fn inline FCLICommandSpec ReadCommandSpec( const FString &Key, const TSharedRef<FJsonObject> &Command) */
...
/** User Story: As a features cli consumer, I need to invoke read command data through a stable signature so the features cli workflow remains explicit and composable. @fn inline TArray<FCLICommandSpec> ReadCommandData( const TSharedRef<FJsonObject> &Commands) */
