...
/**
 * Executes one raw Unreal commandlet invocation through the SDK CLI surface.
 * User Story: As an Unreal command host, I need one SDK call that owns CLI
 * parsing, validation, dispatch, presentation, and authored exit codes.
 * @fn FORBOCAI_SDK_API int32 ExecuteInvocation(const FString &Params)
 */
FORBOCAI_SDK_API int32 ExecuteInvocation(const FString &Params);
