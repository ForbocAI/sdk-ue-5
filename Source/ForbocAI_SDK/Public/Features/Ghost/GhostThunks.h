  ...
  return [SessionId](std::function<AnyAction(const AnyAction &)> Dispatch,
    ...
    return ApiKeyError.hasValue
        ...
        [Dispatch](const FGhostResults &Response) {
          ...
          AddResults::apply(Response.Tests, Report.Results, 0);
...
/** User Story: As a features ghost consumer, I need to invoke get ghost history thunk through a stable signature so the features ghost workflow remains explicit and composable. @fn inline ThunkAction<TArray<FGhostHistoryEntry>, FRuntimeState> getGhostHistoryThunk(int32 Limit = 10) */
