...
/** User Story: As a features cli consumer, I need to invoke select cli command matrix through a stable signature so the features cli workflow remains explicit and composable. @fn inline const TArray<FCommandSpec> &selectCliCommandMatrix( const FCLIState &State) */
inline const TArray<FCommandSpec> &selectCliCommandMatrix(
...
/** User Story: As a features cli consumer, I need to invoke has surface through a stable signature so the features cli workflow remains explicit and composable. @fn inline bool HasSurface(const FCommandSpec &Command, const FString &Surface) */
inline bool HasSurface(const FCommandSpec &Command,
...
inline TArray<FString> selectCliCommandKeys(const FCLIState &State,
                                            const FString &Surface) {
  return func::map_array<FCommandSpec, FString>(
      func::filter_array<FCommandSpec>(
          selectCliCommandMatrix(State),
          [&Surface](const FCommandSpec &Command) {
            return HasSurface(Command, Surface);
          }),
      [](const FCommandSpec &Command) { return Command.Key; });
}

/** User Story: As a features cli consumer, I need to invoke get cli command spec recursive through a stable signature so the features cli workflow remains explicit and composable. @fn inline func::Maybe<FCommandSpec> getCliCommandSpecRecursive( const FCLIState &State, const FString &Key, int32 Index) */
inline func::Maybe<FCommandSpec> getCliCommandSpecRecursive(
    const FCLIState &State, const FString &Key, int32 Index) {
  return Index >= State.Commands.Num()
             ? func::nothing<FCommandSpec>()
             : State.Commands[Index].Key == Key
                   ? func::just<FCommandSpec>(State.Commands[Index])
                   : getCliCommandSpecRecursive(
                         State, Key, Index + State.Parsing.NextIndexOffset);
}

/** User Story: As a features cli consumer, I need to invoke get cli command spec through a stable signature so the features cli workflow remains explicit and composable. @fn inline func::Maybe<FCommandSpec> getCliCommandSpec( const FCLIState &State, const FString &Key) */
inline func::Maybe<FCommandSpec> getCliCommandSpec(
...
inline bool isValidCommandKey(const FCLIState &State,
                              const FString &Key) {
  const func::Maybe<FCommandSpec> Command = getCliCommandSpec(State, Key);
