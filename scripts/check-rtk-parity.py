#!/usr/bin/env python3
"""Check UE Core/rtk.hpp against installed @reduxjs/toolkit exports.
...
extracts names present in UE Core/rtk.hpp, updates the SDK map, and fails while
any exported name is missing from rtk.hpp.
"""
...
def rtk_hpp_names(path: Path) -> tuple[str, ...]:
    text = strip_comments(read_text(path))
    names: set[str] = set()
    for pattern in (
...
def build_section(
    ...
    lines = [
        ...
        "",
        f"Generated by `scripts/check-rtk-parity.py` on {generated_on}.",
        "",
        f"- Toolkit source: `@reduxjs/toolkit` {version} from `{ts_root}`",
        f"- UE mirror: `{ue_root / 'Source/ForbocAI_SDK/Public/Core/rtk.hpp'}`",
        "- This section is export-name derived. It is intentionally strict: `rtk.hpp` is not exact while any exported Toolkit or RTK Query name is missing.",
        "",
    ]
    for group in groups:
...
def main() -> int:
    ...
    ue_names = rtk_hpp_names(rtk_hpp)
    section, missing_total = build_section(version, ts_root, ue_root, groups, ue_names)
    ...
    print(f"[info] UE rtk.hpp names discovered: {len(ue_names)}")
    for group in groups:
    ...
    if missing_total:
        print(f"[fail] rtk.hpp is missing {missing_total} exported Redux Toolkit / RTK Query names.")
        return 1
