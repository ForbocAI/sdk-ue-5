            ...
            *table(
                ...
                [
                    (
                        ...
                        "No TS source folder has the same normalized path/name signature in this parity program.",
                    )
            ...
            *table(
                ...
                [
                    (
                        md_code(path),
                        md_code(normalize_path_signature(path)),
                        "No TS source file has the same normalized path/name signature in this parity program.",
                    )
            ...
            *table(
                ...
                [
                    (
                        ...
                        md(symbol.kind),
                        "No exported TS symbol/function has the same normalized name in this parity program.",
                    )
...
def build_generated_section(
    ...
    lines: list[str] = [
        ...
        "",
        "Generated by `scripts/check-sdk-parity.py` from the source trees named below.",
        "",
...
def strip_historical_sections(text: str) -> str:
    """Keep the generated architecture map forward-facing."""
    history_heading = "\n## 12. Change Log"
    return (
        text.split(history_heading, maxsplit=1)[0].rstrip() + "\n"
        if history_heading in text
        else text
    )
...
def main() -> int:
    ...
    if map_path is None:
        ...
        updated = replace_generated_section(
            strip_historical_sections(original), section
        )
